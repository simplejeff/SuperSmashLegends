...
public class Player<T extends PhysicsEntity<T>> extends GamePhysicsEntity<T>{
  ...
  GameWorld<T> myGW;
  public Player(GameWorld<T> gw, Vec2f startLoc, float width, float length) {
    myGW = gw;
  ...
  public void doCollide(T e) {
    ((GamePhysicsEntity<T>)e).collidePlayer(this);
  }
  ...
  public void onTick(long millisSincePreviousTick) {
    ...
    if(touchingGround && ((!keys.contains(KeyEvent.VK_A) && !keys.contains(KeyEvent.VK_D)) || frozen)) {
    ...
    if(knockedUp && debuffTimer > 1400.0) {
    ...
    if(keys.contains(KeyEvent.VK_S)) {
      ...
      if(keys.contains(KeyEvent.VK_W)) {
        if(jumpCount > 0 && jumpReload < 0.1f) {
         ...
         // updateImpulse(new Vec2f(0, -500f/maxMass));
        }
      }
      ...
      if(keys.contains(KeyEvent.VK_G)) {
        if(!poisoned && !reloadPrimary && !crouching) {
            ...
            if(character < 2) {
              totalMillis = 0;
              attack = 1;
            myGW.addProjectileOne((T)new Bullet<T>(myGW, s.getCenter(), bulletDir, true, facing, character));
            } else {
              totalMillis = 0;
              attack = 2;
              myGW.addProjectileOne((T)new Punch<T>(myGW, s.getCenter(), bulletDir, true, facing));
            }
            primary = 1000.0f;
            reloadPrimary = true;
        }
      ...
      if(keys.contains(KeyEvent.VK_H)) {
        if(!poisoned && !reloadSecondary && !crouching) {
            ...
            if(character == 0) {
              totalMillis = 0;
              attack = 1;
              myGW.addProjectileOne((T)new Poison<T>(myGW, s.getCenter(), bulletDir, true, facing));
            } else if (character == 1) {
              totalMillis = 0;
              attack = 1;
             ...
             //myGW.addProjectileOne((T)new Blink<T>(myGW, s.getCenter(), bulletDir, true));
              ...
              totalMillis = 0;
              attack = 2;
              myGW.addProjectileOne((T)new Tornado<T>(myGW, new Vec2i(s.getLocation().x-40, s.getLocation().y-80), bulletDir, true));
            } else {
              totalMillis = 0;
              attack = 2;
              myGW.addProjectileOne((T)new Freeze<T>(myGW, s.getCenter(), bulletDir, true, facing));
            }
            secondary = 5000.0f;
  ...
  public void onKeyPressed(KeyEvent e) {
   // shield.onKeyPressed(e);
    ...
    case KeyEvent.VK_W:
      if(touchingGround) {
        touchingGround = false;
        updateImpulse(new Vec2f(0, -20));
      }
      break;
    case KeyEvent.VK_A:
      facing = -1;
      vel = vel.plus(new Vec2f(-2f/maxMass, 0));
      if(touchingGround) {
        vel = new Vec2f(-.3f, vel.y);
      }
      break;
    ...
    case KeyEvent.VK_D:
      ...
      break;
    case KeyEvent.VK_G:
      if(!poisoned && !reloadPrimary) {
        System.out.println("leggo");
          Vec2f bulletDir = new Vec2f(facing, 0);
          myGW.addProjectileOne((T)new Bullet<T>(myGW, s.getCenter(), bulletDir));
          primary = 500.0f;
          reloadPrimary = true;
      }
    default:
